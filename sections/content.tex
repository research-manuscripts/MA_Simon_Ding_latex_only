...
Es gibt verschiedene Arten von Softwaretests, darunter Unit-Tests, Systemtests und Akzeptanztests.
\cite{Sommerville10}
...
Gleichzeitig bedeuten Änderungen an der grafischen Benutzeroberfläche (Englisch: graphical user interface, GUI) oft, dass auch die Tests geändert werden müssen.
\cite{ChallengesSelenium}
...
Es wird aufgefordert, eine Benutzerinteraktion zu generieren, die anschließend automatisch in der Anwendung ausgeführt wird.
\cite{GPT3Testing}
...
Dies wurde zuvor von \Citeauthor{GPT3Testing} untersucht \cite{GPT3Testing}.
...
Scheinbar grundlegend für diese Anwendung ist die Fähigkeit des LLMs, die grafische Benutzeroberfläche (Englisch: graphical user interface, GUI) der zu testenden Anwendung zu verstehen und zu navigieren.
Dabei wäre nicht nur die Repräsentation der GUI als HTML-Dokument, einer Baumstruktur, von Bedeutung, sondern auch die Graphstruktur, die sich aus der Verknüpfung der einzelnen Seiten und den Zustände der Anwendung ergibt.
Die Kognitionswissenschaft liegt allerdings nahe, dass zum Verständnis solcher Graphstrukturen eine Art von räumlichem Denken erforderlich ist \cite{what_is_a_cognitive_map}, das bei LLMs scheinbar nicht vorhanden ist \cite{cogmaps_llm}.
...
Diese Arbeit folgt dem Ansatz von \Citeauthor{GPT3Testing}, eine strukturierte Darstellung der Benutzeroberfläche der zu testenden Anwendung zusammen mit einer natürlichsprachlichen Testbeschreibung als Prompt für ein LLM-Modell zu verwenden \cite{GPT3Testing}.
...
\section{Transformer und Sprachmodelle: Ein Überblick}
